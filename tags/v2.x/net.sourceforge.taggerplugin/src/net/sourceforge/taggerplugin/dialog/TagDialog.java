...
public class TagDialog extends Dialog {
    ...
    private String tagName, tagDescription, tagContainerId;
    private String[] tagContainers;
    private Text nameTxt,descTxt,errorMessageText;
    private Combo containerCombo;
    private boolean tagContainerEditable;
    ...
    public void setTagContainerEditable(boolean tagContainerEditable) {
		this.tagContainerEditable = tagContainerEditable;
	}
    
    public void setTagContainers(String[] tagContainers) {
		this.tagContainers = tagContainers;
	}
    
    public void setTagContainerId(String tagContainerId) {
		this.tagContainerId = tagContainerId;
	}
    
    public String getTagContainerId() {
		return tagContainerId;
	}
    ...
    protected void buttonPressed(int buttonId) {
        if (buttonId == IDialogConstants.OK_ID) {
        	tagContainerId = containerCombo.getText();
        	tagName = nameTxt.getText();
        	tagDescription = descTxt.getText();
        } else {
        	tagContainerId = null;
        	tagName = null;
        	tagDescription = null;
    ...
    protected void createButtonsForButtonBar(Composite parent) {
        ...
        if(tagContainerId != null){
        	containerCombo.setText(tagContainerId);
        }
        if(tagName != null){
        	nameTxt.setText(tagName);
        }
        if(tagDescription != null){
        	descTxt.setText(tagDescription);
        }
    }
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        createContainerLabel(composite, labelFont);
        createContainerCombo(composite);
	...
	private void createContainerLabel(final Composite composite, final Font labelFont){
		final Label containerLbl = new Label(composite,SWT.LEFT);
        containerLbl.setText(TaggerMessages.TagDialog_Label_Container);
        containerLbl.setFont(labelFont);
	}
	
	private void createContainerCombo(final Composite composite) {
		containerCombo = new Combo(composite,SWT.DROP_DOWN | SWT.READ_ONLY);
		containerCombo.setItems(tagContainers);
		containerCombo.setEnabled(tagContainerEditable);
		
        final GridData containerData = new GridData(GridData.GRAB_HORIZONTAL | GridData.HORIZONTAL_ALIGN_FILL);
        containerData.widthHint = 200;
        containerCombo.setLayoutData(containerData);
	}
